...
export default function CityTiles(props){
    ...
    function DashboardList(){
        ...
        return(
            <Row xs='auto' md='auto' lg='auto' className="m-auto g-4 text-center center--items">
                {tiles}
            </Row>
        );       
    }
    ...
    function CityCard(cityprops){
        ...
        return(
            <Col>
                <Card id={cityprops.city.id} className='mt-1 mb-1 my--card' text='light' border='light' style={{width:'16rem'}}>
                    <Card.Body>
                        <Card.Title className='bold m-auto spreadout' as="h1">
                            
                            <button className='icon' onClick={()=>DeleteCity(cityprops.city.id)}><img className='icon--x' src={x} alt='delete {cityprops.city.name} weather tile'/></button>

                            <button className='icon' onClick={()=> UpdateCity(cityprops.city.id, cityprops.city.name)}><img className='icon--update' src={update} alt='update {cityprops.city.name} weather'/></button>
                        ...
                        <Card.Subtitle className='bold text-center' as="h1">
                            {Math.round(cityprops.city.main.temp)}<sup>o</sup>
                        </Card.Subtitle>
                        : null}
                        ...
                        <Card.Text as='div'>
                            ...
                            <h3 className='text-center mb-0'>{cityprops.city.weather[0].description}</h3> : null }
                        
                            {cityprops.city.weather ?
                            <img className='weather--icon m-auto' src={`https://openweathermap.org/img/wn/${cityprops.city.weather[0].icon}@2x.png`} alt='weather clouds icon'/>: null }
                    
                            {cityprops.city.main? 
                            <p className='text-center mb-0'>Humidity: {Math.round(cityprops.city.main.humidity)}%</p>: null}

                            {cityprops.city.weather ? 
                                <p className='text-center mb-0'>Wind: {Math.round(cityprops.city.wind.speed)} mph</p>: null } 
